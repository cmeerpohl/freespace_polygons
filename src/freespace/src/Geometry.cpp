...
#include "k2i/utils/Convenience.h"
              ...
              double direction) noexcept
{
    ...
    auto normalizedDirection = utils::ANGLE_NORMALIZE_INPLACE(direction);
    auto difference = angle - normalizedDirection;
    if (std::fabs(utils::ANGLE_NORMALIZE_INPLACE(difference)) <= M_PI_2 + 1E-6)
    {
