...
OccupancyMap2D::OccupancyMap2D() : m_values{}, m_origin{},m_resolution{.0}, m_width{0}, m_height{0}
{
}

OccupancyMap2D::OccupancyMap2D(std::vector<int8_t> values, types::PoseSE2 origin, double resolution, size_t size)
    : m_values(std::move(values)), m_origin(std::move(origin)), m_resolution(resolution), m_width(size), m_height(size)
{
}
...
types::Point2D OccupancyMap2D::transformPositionFromMap(const types::Point2D &position) const noexcept
{
    ...
    return {globalPose.x(), globalPose.y()};
}
